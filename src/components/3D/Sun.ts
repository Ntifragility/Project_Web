 ...
 * Scaled up to 1.0x baseline size (2x larger than the previous 0.5x scale).
 */
...
export function createRealisticSun(scene: THREE.Scene, position: THREE.Vector3): RealisticSunInstance {
    ...
    // 1. Core Photosphere (Radius 2.0 * 1.0 = 2.0)
    ...
    // 2. Solar Prominences (Scaled 1.0x, 0.25x brighter)
    ...
    for (let i = 0; i < numProminences; i++) {
        ...
        const promMat = new THREE.ShaderMaterial({
            ...
            vertexShader: `
                ...
                void main() {
                    vUv = uv;
                    vec3 p = position;
                    // Magnetic loop turbulent displacement scaled 1.0x
        ...
        // Position on scaled Sun's surface perimeter (r = 1.95)
    ...
    // 3. Coronal Mass Ejection (CME) Volumetric Jet Shell (2.8 * 1.0 = 2.8)
    ...
    const cmeMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                // Pulsing explosive expansion along noise channels scaled 1.0x
    ...
    // 4. Optical Flare Billboard (Camera-facing Corona Glow) (12.0 * 1.0 = 12.0)
